...
export async function POST(req: NextRequest) {
  ...
  const rateLimit = await consumeRateLimit({
    key: `import:finance-vendors:${auth.user.id}:${getRequestIp(req)}`,
    limit: 10,
    windowMs: 10 * 60 * 1000,
  });
  if (!rateLimit.allowed) {
    writeAuditLog({
      action: "finance.import_vendors",
      status: "denied",
      request: req,
      actorUserId: auth.user.id,
      actorRole: auth.user.roleName,
      tenantId: auth.user.tenantId,
      message: "Vendor import rate limit exceeded",
    });
  ...
  if (!rows.length) {
    writeAuditLog({
      ...
      actorRole: auth.user.roleName,
      tenantId: auth.user.tenantId,
      message: "Vendor import rows empty",
    });
